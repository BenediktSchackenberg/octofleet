<#
.SYNOPSIS
    Octofleet Agent Full Installer - Downloads, installs, configures, starts
...
.PARAMETER GatewayUrl
    The Octofleet Gateway URL (e.g., http://192.168.0.5:18789)
...
.EXAMPLE
    ...
    .\Install-OctofleetAgent.ps1 -GatewayUrl "http://192.168.0.5:18789" -GatewayToken "abc123"

    # Update existing installation (keeps config):
    .\Install-OctofleetAgent.ps1

    # With enrollment token:
    .\Install-OctofleetAgent.ps1 -EnrollToken "abc123xyz..."
...
Write-Host "  Octofleet Agent - Full Installer" -ForegroundColor Cyan
...
if (-not $hasNewCredentials -and $existingConfig) {
    ...
    Write-Host "  .\Install-OctofleetAgent.ps1 -GatewayUrl 'http://...' -GatewayToken 'abc...'" -ForegroundColor Gray
    Write-Host ""
    Write-Host "Or use enrollment token:" -ForegroundColor Yellow
    Write-Host "  .\Install-OctofleetAgent.ps1 -EnrollToken 'xyz...'" -ForegroundColor Gray
    exit 1
}
...
$tempDir = Join-Path $env:TEMP "OctofleetInstall_$(Get-Random)"
...
if ($PackageUrl) {
    ...
    try {
        ...
        $headers = @{ "User-Agent" = "Octofleet-Installer" }
    ...
    if (-not $downloadUrl) {
        if ($Version -eq "latest") {
            $downloadUrl = "https://github.com/$GitHubRepo/releases/latest/download/OctofleetAgent.Service.zip"
        } else {
            $downloadUrl = "https://github.com/$GitHubRepo/releases/download/v$Version/OctofleetAgent.Service.zip"
        }
...
try {
    ...
    Write-Host "  1. Build: dotnet publish src/OctofleetAgent.Service -c Release -o publish" -ForegroundColor Gray
    Write-Host "  2. ZIP the 'publish' folder as 'OctofleetAgent.Service.zip'" -ForegroundColor Gray
...
if (-not $existingService) {
    # Create new service
    $result = sc.exe create $ServiceName binPath="$servicePath" start=auto DisplayName="Octofleet Agent"
