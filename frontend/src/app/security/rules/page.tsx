...
export default function RulesPage() {
  ...
  const save = async () => {
    try {
      ...
      await apiClient.post(`/security/rules`, body, { showErrorToast: false });
      setShowForm(false);
  ...
  const deleteRule = async (id: string) => {
    await apiClient.delete(`/security/rules/${id}`, { showErrorToast: false });
    load();
  };
  ...
  return (
    <div className="p-6 space-y-6">
      ...
      {evalResult && (
        <div className="bg-zinc-800 border border-border rounded p-3 text-sm">
      ...
      <div className="space-y-3">
        {rules.map(r => (
          <div key={r.id} className="bg-zinc-800 border border-border rounded-lg p-4">
        ...
        {rules.length === 0 && <div className="text-center text-muted-foreground py-8">No behavior rules defined yet</div>}
      </div>
    </div>
  );
}
