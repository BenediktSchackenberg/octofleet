  ...
  return (
    <div className="p-6 space-y-6">
        ...
        <div className="space-y-4">
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
            <div className="bg-zinc-800 rounded-lg border border-border p-4">
              <h3 className="text-sm font-semibold text-muted-foreground mb-2">Operations Distribution</h3>
              {(fileData.operationsDistribution || []).map((o: any) => (
                <div key={o.operation} className="flex justify-between text-sm py-1 border-b border-border">
            ...
            <div className="bg-zinc-800 rounded-lg border border-border p-4">
              <h3 className="text-sm font-semibold text-muted-foreground mb-2">Top Users</h3>
              {(fileData.topUsers || []).map((u: any) => (
                <div key={u.username} className="flex justify-between text-sm py-1 border-b border-border">
                  <span className="text-muted-foreground">{u.username}</span>
                  <span className="text-white font-mono">{u.count}</span>
                </div>
              ))}
              {(!fileData.topUsers || fileData.topUsers.length === 0) && <p className="text-muted-foreground text-sm">No user data yet</p>}
            </div>
          </div>

          <div className="bg-zinc-800 rounded-lg border border-border p-4">
            <h3 className="text-sm font-semibold text-muted-foreground mb-2">Top Paths</h3>
            <div className="max-h-64 overflow-y-auto space-y-1">
              {(fileData.topPaths || []).map((p: any, i: number) => (
                <div key={i} className="flex justify-between text-sm py-1 border-b border-border">
                  <span className="text-muted-foreground font-mono text-xs truncate max-w-[70%]">{p.path}</span>
          ...
          <div className="bg-zinc-800 rounded-lg border border-border p-4">
            <h3 className="text-sm font-semibold text-muted-foreground mb-2">Recent Events</h3>
            <div className="max-h-80 overflow-y-auto">
              <table className="w-full text-xs">
                <thead className="text-muted-foreground"><tr><th className="text-left pb-1">Time</th><th className="text-left pb-1">Node</th><th className="text-left pb-1">Type</th><th className="text-left pb-1">Path</th><th className="text-left pb-1">User</th></tr></thead>
      ...
      {tab === "users" && userData && (
        <div className="space-y-4">
          <div className="bg-zinc-800 rounded-lg border border-border p-4">
            <h3 className="text-sm font-semibold text-muted-foreground mb-2">User Summary</h3>
            <table className="w-full text-sm">
              <thead className="text-muted-foreground text-left"><tr><th className="pb-2">User</th><th className="pb-2">Events</th><th className="pb-2">Unique Ops</th><th className="pb-2">Unique Files</th><th className="pb-2">Last Active</th></tr></thead>
              <tbody className="text-muted-foreground">
            ...
            {(!userData.users || userData.users.length === 0) && <p className="text-muted-foreground text-sm py-4 text-center">No user activity data yet</p>}
          </div>
          ...
          {userData.sensitiveAccess?.length > 0 && (
            <div className="bg-yellow-900/20 border border-yellow-800 rounded-lg p-4">
              ...
              <table className="w-full text-sm">
                <thead className="text-muted-foreground text-left"><tr><th className="pb-1">User</th><th className="pb-1">Path</th><th className="pb-1">Count</th></tr></thead>
                <tbody className="text-muted-foreground">
