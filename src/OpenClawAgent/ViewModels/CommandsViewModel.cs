...
/// <summary>
/// Commands view model - OpenClaw command terminal
/// </summary>
public partial class CommandsViewModel : ObservableObject
{
    ...
    public CommandsViewModel()
    {
        ...
        CommandHistory.Add(new CommandOutput
        {
            ...
            Text = "OpenClaw Command Terminal\nType 'help' for available commands.\n",
            IsSystem = true
        });
    }
    ...
    private async Task ExecuteCommandAsync()
    {
        ...
        try
        {
            var result = await ExecuteOpenClawCommandAsync(command);
            CommandHistory.Add(new CommandOutput
            {
                Timestamp = DateTime.Now,
                Text = result.Output,
    ...
    private async Task<CommandResult> ExecuteOpenClawCommandAsync(string command)
    {
...
OpenClaw CLI commands are also supported (e.g., 'openclaw status')"
            };
        }
  ...
  Last Sync:    Never"
    ...
    private async Task<CommandResult> ExecuteCliCommandAsync(string command)
    {
        try
        {
            ...
            {
                FileName = "openclaw",
                Arguments = command.StartsWith("openclaw ") ? command.Substring(9) : command,
