...
public class JobPoller : BackgroundService
{
    ...
    private async Task ScheduleRebootAsync(int delaySeconds, CancellationToken ct)
    {
        try
        {
            // shutdown /r /t <seconds> /c "Octofleet scheduled reboot"
            var psi = new ProcessStartInfo
            {
            ...
            psi.ArgumentList.Add("Octofleet Agent: Scheduled reboot after job completion");
    ...
    /// <summary>
    /// Restarts the Octofleet Agent service by spawning an external process
    ...
    private async Task<CommandResult> ExecuteRestartAgentAsync(CancellationToken ct)
    {
        ...
        const string serviceName = "OctofleetNodeAgent";
...
";
        ...
        var scriptPath = Path.Combine(Path.GetTempPath(), $"octofleet-restart-{Guid.NewGuid():N}.ps1");
