...
public class NodeWorker : BackgroundService
{
    ...
    {
        _logger.LogInformation("Octofleet Node Agent starting...");
        ...
        {
            _logger.LogWarning("Service not configured. Please configure via the Octofleet Agent GUI.");
        ...
        _logger.LogInformation("Octofleet Node Agent stopped.");
    }
    ...
    private async Task ConnectAndRunAsync(ServiceConfig config, CancellationToken ct)
    {
        ...
        var request = new
        {
            ...
            @params = new
            {
                ...
                device = deviceObject,
                userAgent = $"octofleet-windows-service/0.3.13 ({config.DisplayName})"
            }
        };
