...
public class AutoUpdater : BackgroundService
{
    ...
    public AutoUpdater(ILogger<AutoUpdater> logger, IHostApplicationLifetime lifetime, ServiceConfig config)
    {
        ...
        _httpClient.DefaultRequestHeaders.Add("User-Agent", "OctofleetAgent-AutoUpdater");
    }
    ...
    {
        var tempDir = Path.Combine(Path.GetTempPath(), $"OctofleetUpdate-{newVersion}-{Guid.NewGuid():N}");
        ...
        try
        {
            ...
            // The ZIP might contain a nested folder like "octofleet-release-X.X.X"
            var sourceDir = extractPath;
            
            // Keep drilling down until we find the exe
            while (true)
            {
                var exePath = Path.Combine(sourceDir, "OctofleetAgent.Service.exe");
                ...
                else if (subdirs.Length > 1)
                {
                    ...
                    var found = subdirs.FirstOrDefault(d => File.Exists(Path.Combine(d, "OctofleetAgent.Service.exe")));
                    if (found != null)
                    {
                        sourceDir = found;
                        break;
            ...
            if (!File.Exists(Path.Combine(sourceDir, "OctofleetAgent.Service.exe")))
            {
                _logger.LogError("Could not find OctofleetAgent.Service.exe in extracted files");
                return;
            }
    ...
    private static string CreateUpdateScript(string sourceDir, string installDir, string version, string tempDir)
    {
        ...
        return $@"
# Octofleet Agent Update Script - v{version}
...
$installDir = '{dst}'
$tempDir = '{tmp}'
$logFile = 'C:\ProgramData\Octofleet\logs\update.log'
...
Copy-Item ""$installDir\OctofleetAgent.Service.exe"" ""$backupDir\OctofleetAgent.Service.exe.bak"" -Force -ErrorAction SilentlyContinue
...
if (Test-Path ""$installDir\OctofleetAgent.Service.exe"") {{
    Log ""Files copied successfully""
}} else {{
    Log ""ERROR: New executable not found, restoring backup...""
    Copy-Item ""$backupDir\OctofleetAgent.Service.exe.bak"" ""$installDir\OctofleetAgent.Service.exe"" -Force -ErrorAction SilentlyContinue
}}
