...
/// <summary>
/// Service for communicating with Octofleet Gateway via WebSocket protocol v3
/// </summary>
public class GatewayService : IDisposable
{
    ...
    private async Task<ConnectResult> SendConnectRequestAsync(ClientWebSocket ws, string? token, string? nonce, CancellationToken ct)
    {
        ...
        // Build connect request following Octofleet Gateway Protocol v3
        ...
        var request = new
        {
            ...
            @params = new
            {
                ...
                client = new
                {
                    id = "octofleet-control-ui",  // Known client ID for control UIs
                    version = "0.3.0",
                    platform = "windows",
                ...
                userAgent = $"octofleet-windows-agent/0.3.0 ({Environment.OSVersion.Platform})"
