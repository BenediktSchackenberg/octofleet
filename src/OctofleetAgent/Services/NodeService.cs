...
/// <summary>
/// Service for registering as a Node with Octofleet Gateway.
...
public class NodeService : IDisposable
{
    ...
    private async Task<NodeConnectResult> SendNodeConnectRequestAsync(ClientWebSocket ws, string? token, string? nonce, string displayName, CancellationToken ct)
    {
        ...
        // Build connect request with role: node (following Octofleet Gateway Protocol v3)
        var request = new
        {
            type = "req",
            id = requestId,
            ...
            @params = new
            {
                ...
                userAgent = $"octofleet-windows-agent/0.3.0 ({displayName})"
            }
        };
