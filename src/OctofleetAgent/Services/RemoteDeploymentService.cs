...
public class RemoteDeploymentService
{
    ...
    public async Task DeployAsync(RemoteHost host, string password, GatewayConfig gateway)
    {
        await Task.Run(() =>
        {
            ...
            if (!isInstalled)
            {
                OnProgress(host, DeploymentStage.Installing, "Installing Octofleet agent...");
                
                // Copy installer to remote host
                ps.AddScript($@"
                    $installerPath = 'C:\Temp\OctofleetAgent-Setup.msi'
            ...
            ps.AddScript($@"
                $configPath = Join-Path $env:APPDATA 'Octofleet\Agent\config.json'
            ...
            ps.AddScript(@"
                ...
                if ($service) {
                    ...
                    $agentPath = Join-Path $env:ProgramFiles 'Octofleet\Agent\OctofleetAgent.exe'
            ...
            try
            {
                ...
                ps.AddScript(@"
                    @{
                        ...
                        AgentRunning = (Get-Process -Name 'OctofleetAgent' -ErrorAction SilentlyContinue) -ne $null
                        ServiceStatus = (Get-Service -Name 'OctofleetAgent' -ErrorAction SilentlyContinue).Status
