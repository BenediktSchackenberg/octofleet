<#
.SYNOPSIS
    Installs or updates the OpenClaw Windows Agent.

.DESCRIPTION
    Downloads the latest release from GitHub and installs the OpenClaw Windows Agent
    as a Windows Service. Supports enrollment tokens for automatic registration.

.PARAMETER GatewayUrl
    WebSocket URL to the OpenClaw Gateway (e.g., ws://192.168.0.5:18789)
...
.PARAMETER InstallPath
    Installation directory. Default: C:\Program Files\OpenClaw
...
.EXAMPLE
    ...
    .\Install-OpenClawAgent.ps1 -GatewayUrl "ws://192.168.0.5:18789" -GatewayToken "mytoken"
...
$ServiceExe = "DIOOpenClawAgent.Service.exe"
...
function Get-LatestRelease {
    ...
    try {
        $release = Invoke-RestMethod -Uri $apiUrl -Headers @{ "User-Agent" = "OpenClaw-Installer" }
        return $release
    }
    catch {
...
function Install-OpenClawAgent {
    # Check admin rights
    if (-not (Test-Administrator)) {
        throw "This script requires Administrator privileges. Please run as Administrator."
    }
    
    Write-Status "OpenClaw Windows Agent Installer" "Info"
    Write-Host ""
    ...
    # Download
    $tempZip = Join-Path $env:TEMP "OpenClawAgent-$version.zip"
    ...
    if ($service -and $service.Status -eq "Running") {
        Write-Status "OpenClaw Agent v$version installed successfully!" "Success"
        Write-Host ""
