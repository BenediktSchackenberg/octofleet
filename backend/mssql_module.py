...
def generate_disk_prep_script(disk_config: DiskConfigSection) -> str:
    ...
    script_parts = [
        ...
        "",
        "# Function to free up a drive letter if used by CD-ROM or other removable",
        ...
        "    if ($existing) {",
        "        Write-Host \"  Drive ${Letter}: is in use (Type: $($existing.DriveType))\"",
        "        ",
        "        # Try to remove if it's CD-ROM or removable",
        "        $partition = Get-Partition -DriveLetter $Letter -ErrorAction SilentlyContinue",
        "        if ($partition) {",
        "            $disk = Get-Disk -Number $partition.DiskNumber -ErrorAction SilentlyContinue",
        "            if ($disk -and $disk.BusType -eq 'ATAPI') {",
        "                Write-Host \"  Removing CD-ROM from ${Letter}:\"",
        ...
        "        }",
        "",
        "        # Check WMI for CD-ROM",
        "        $cdrom = Get-WmiObject Win32_CDROMDrive | Where-Object { $_.Drive -eq \"${Letter}:\" }",
        "        if ($cdrom) {",
        "            Write-Host \"  Found CD-ROM on ${Letter}: - removing drive letter\"",
        ...
        "        }",
        "",
        "        Write-Host \"  Warning: ${Letter}: is in use by another volume\"",
        "        return $false",
        "    }",
        "    return $true",
        "}",
        ...
        "",
        "    # First, free up the drive letter if needed",
        "    $freed = Free-DriveLetter -Letter $DriveLetter",
        "    if (-not $freed) {",
        "        throw \"Cannot use drive letter ${DriveLetter}: - already in use\"",
        "    }",
        "",
